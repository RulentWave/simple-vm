...
cell_t* add_cell_to_tape(tape_t* tape, char direction) {
	...
	*new_cell = (cell_t){.symbol = '_', // Default blank symbol
	                     .left = NULL,
	                     .right = NULL};
...
machine_t* create_machine_from_toml(char* inputbuf) {
	...
	// For every transition rule, next_state will point to NULL if the
	// transition's next_state_name doesn't refer to a valid state because all
	// transitions are initialized with a NULL pointer for next_state
