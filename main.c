...
int main() {
	...
	while (true) {
		if (!machine->current_state) {
			printf("current_state points to NULL. halting...\n");
			break;
		} // Halt if state not found
		if (strcmp(machine->current_state->name, "halt") == 0) {
			printf("halting...\n");
			break;
		} // Halt if state name is "halt"
		...
		if (!t) {
			...
			break;
		} // Halt if no matching transition
